...
if '--copy_web_only' not in sys.argv:
  ...
  print(timestamp(), 'trying MulticoreTSNE')
  try:
  ...
  print(timestamp(), 'trying hdbscan')
  try:
...
def process_images(**kwargs):
  ...
  print(timestamp(), 'process_images')
  sys.stdout.flush()
  ...
  kwargs['vecs'] = get_custom_vectors(**kwargs)
  get_manifest(**kwargs)
  write_images(**kwargs)
...
def filter_images(**kwargs):
  ...
  f = open(kwargs['images']) # Open file on read mode
  image_paths = f.read().splitlines() # List with stripped line-breaks
  f.close() # Close file
...
def get_manifest(**kwargs):
  ...
  print(timestamp(), 'Creating manifest')
  sys.stdout.flush()
  # load the atlas data
  ...
  # remove full path to the output directory from manifest
  # BEFORE = D:\\Dev\\ISYNC\\AstroView\\App\\Storage\\DataSets\\meerkat\\maps\\featdata_dinov2_vitl14\\data\\atlases\\d5eda55e-d95e-11ef-9a43-5ce0c5a3621d
  # AFTER = data\\atlases\\d5eda55e-d95e-11ef-9a43-5ce0c5a3621d
  path_to_remove = kwargs['out_dir'][ : -4]; #remove 'data' suffix
  manifest['layouts']['umap']['variants'][0]['layout'] = manifest['layouts']['umap']['variants'][0]['layout'].replace(path_to_remove,'')
  manifest['layouts']['umap']['variants'][0]['jittered'] = manifest['layouts']['umap']['variants'][0]['jittered'].replace(path_to_remove,'')
  manifest['layouts']['alphabetic']['layout'] = manifest['layouts']['alphabetic']['layout'].replace(path_to_remove,'')
  manifest['imagelist'] = manifest['imagelist'].replace(path_to_remove,'')
  manifest['atlas_dir'] = manifest['atlas_dir'].replace(path_to_remove,'')
  manifest['default_hotspots'] = manifest['default_hotspots'].replace(path_to_remove,'')
  manifest['custom_hotspots'] = manifest['custom_hotspots'].replace(path_to_remove,'')
  ...
  imagelist = {
    'cell_sizes': sizes,
    'images': kwargs['image_paths'],
    'atlas': {
      'count': len(atlas_ids),
      'positions': pos,
    },
  }
  write_json(path_to_remove + manifest['imagelist'], imagelist, **kwargs)
...
def get_custom_vectors(**kwargs):
  '''Loads already calculated features from a json file'''
  print(timestamp(), 'Loading features from json file {} images'.format(len(kwargs['image_paths'])))
  vector_dir = os.path.join(kwargs['out_dir'], 'image-vectors', 'inception')
  if not os.path.exists(vector_dir): os.makedirs(vector_dir)
  
  print(timestamp(), 'Parsing json file')
  featuresdict = {}
  with open(kwargs['features']) as json_data:
    d = json.load(json_data)
    for key in d:
        featuresdict[key] = d[key]
...
def get_hotspots(layouts={}, use_high_dimensional_vectors=True, **kwargs):
  ...
  for idx, i in enumerate(z.labels_):
    if i != -1:
      ...
      d[i]['img'] = kwargs['image_paths'][idx]
      d[i]['img_name'] = clean_filename(kwargs['image_paths'][idx])
...
def write_images(**kwargs):
  ...
  print(timestamp(), 'Writing thumbnails')
  sys.stdout.flush()
  for i in stream_images(**kwargs):
...
def parse():
  ...
  print(timestamp(), 'Parsing started')
  sys.stdout.flush()
  description = 'Create the data required to create a PixPlot viewer'
  parser = argparse.ArgumentParser(description=description, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
  parser.add_argument('--images', '-i', type=str, default=config['images'], help='text file containing image paths', required=False)
  parser.add_argument('--features', '-f', type=str, default=config['features'], help='text file containing image features', required=False)
