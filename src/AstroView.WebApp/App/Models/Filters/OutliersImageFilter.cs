...
public class OutliersImageFilter
{
    ...
    public IQueryable<OutlierDbe> Apply(IQueryable<OutlierDbe> query)
    {
        var name = Name?.Trim();
        ...
        if (MinScore != null)
            query = query.Where(r => r.Score >= MinScore);

        if (MaxScore != null)
            query = query.Where(r => r.Score <= MaxScore);
