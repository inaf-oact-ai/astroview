...
public class SimilarsImageFilter
{
    ...
    public bool ApplyScoreFilterOnChildren { get; set; }

    public List<int> LabelIds { get; set; }

    public SimilarsImageFilter()
    {
        Name = "";
        Telescope = "";
        Survey = "";
        ...
        ApplyScoreFilterOnChildren = true;
    }
    ...
    public string GetDescription(List<LabelDbe> allLabels)
    {
        ...
        if (MinScore != null)
            builder.Append($"HighestScore>={MinScore};");
        if (MaxScore != null)
            builder.Append($"HighestScore<={MaxScore};");
        if (ApplyScoreFilterOnChildren)
            builder.Append($"ApplyScoreFilterOnChildren;");
    ...
    public IQueryable<SimilarDbe> Apply(IQueryable<SimilarDbe> query)
    {
        var name = Name?.Trim();
        ...
        if (MinScore != null)
            query = query.Where(r => r.HighestScore >= MinScore);

        if (MaxScore != null)
            query = query.Where(r => r.HighestScore <= MaxScore);
