...
Data.prototype.parseManifest = function(json) {
  this.json = json;
  json.imageNames = [];
  // json.images = json.images.map(image => image.replace('D:\\Dev\\ISYNC\\AstroView\\App\\Storage\\', ''));
  json.images.forEach(image => {
    var slashIndex = image.lastIndexOf('/');
    var filename = image.substring(slashIndex + 1)
    json.imageNames.push(filename);
  })
  ...
  if (window.astroview.customLayout.length > 0) {
    console.log('Enabling custom layout: ' + window.astroview.customLayout);

    data.json.initial_layout = 'custom';
    this.layouts.custom = {
      layout: window.astroview.customLayout,
    }
  }

  // enable custom hotspots
  if (window.astroview.customHospots.length > 0) {
    console.log('Enabling custom hotspots: ' + window.astroview.customHospots);

    data.json.custom_hotspots = window.astroview.customHospots;
  }
...
World.prototype.getHeightMap = function(callback) {
  ...
  img.src = this.heightmap || 'pixplot/images/heightmap.jpg';
}
...
World.prototype.flyToCellImage = function (img) {
  let imgName = getName(img);
  if (imgName.toUpperCase().indexOf('.FITS')) {
    imgName = imgName.replace(/\.fits/ig, '\.png');
  }
  var idx = null;
  for (var i = 0; i < data.json.images.length; i++) {
    if (data.json.imageNames[i] == imgName)
      idx = i;
  }
  if (!idx)
    return console.warn('The requested image could not be found');
...
Lasso.prototype.addModalEventListeners = function() {
  ...
  this.elems.viewSelected.addEventListener('click', function(e) {
    const selectedImages = Object.keys(this.selected).filter(k => this.selected[k]);
    const selectedImageNames = selectedImages.map(image => image.replace(/^.*[\\/]/, ''));

    // count labels of selected images
    const labelsDic = {};
    selectedImages.forEach(image => {
      const imageLabels = labelManager.indexes.imageLabels[image];
    ...
    const labelStats = [];
    for (let label in labelsDic) {
      labelStats.push({
        name: label,
        count: labelsDic[label],
      })
    }

    // render template
    const template = _.template(this.elems.modalTemplate.textContent);
    ...
    // enable remove label buttons
    const removeButtons = this.elems.modalContainer.querySelectorAll('.remove-label-button');
    for (let i = 0; i < removeButtons.length; i++) {
      const removeButton = removeButtons[i];
      removeButton.addEventListener('click', el => {
        const labelName = removeButton.getAttribute('data-label');
        const allImagesHavingLabel = labelManager.indexes.labelImages[labelName];
        const selectedImagesHavingLabel = _.intersection(allImagesHavingLabel, selectedImages);
        labelManager.removeLabel(labelName, selectedImagesHavingLabel, percent => {
          removeButton.innerHTML = 'Please wait (' + percent.toFixed(2) + '%)';
        }, () => {
          // re-initialize this modal
          removeButton.innerHTML = 'x';
          lasso.elems.viewSelected.click();
        }, err => {
          alert(err.message);
          removeButton.innerHTML = 'x';
        });
    ...
    // enable add label button
    const addButton = this.elems.modalContainer.querySelector('.apply-label-button');
    addButton.addEventListener('click', () => {
      const labelName = this.elems.modalContainer.querySelector('#label-dropdown').value;
      const allImagesHavingLabel = labelManager.indexes.labelImages[labelName];
      const selectedImagesWithoutLabel = _.difference(selectedImages, allImagesHavingLabel);
      labelManager.applyLabel(labelName, selectedImagesWithoutLabel, percent => {
        addButton.innerHTML = 'Please wait (' + percent.toFixed(2) + '%)';
      }, () => {
        // re-initialize this modal
    ...
    // enable create dataset button
    const createDatasetButton = this.elems.modalContainer.querySelector('.create-dataset-button');
    createDatasetButton.addEventListener('click', () => {
      const createDatasetName = this.elems.modalContainer.querySelector('#create-dataset-name');
      if (!createDatasetName.value)
        return;
      datasetManager.createDataset(createDatasetName.value, datasetId => {
        datasetManager.addImages(datasetId, window.astroview.datasetId, selectedImages, percent => {
          createDatasetButton.innerHTML = 'Please wait (' + percent.toFixed(2) + '%)';
        }, () => {
          createDatasetButton.innerHTML = 'Create Dataset';
        }, err => {
          alert(err.message);
          createDatasetButton.innerHTML = 'Create Dataset';
        });
...
Lasso.prototype.setNSelected = function(n) {
  ...
  elem = document.querySelector('#create-dataset-n-images-selected');
  if (elem) {
    elem.textContent = n;
  }
}
...
Modal.prototype.showCells = function(cellIndices, cellIdx) {
  ...
  var src = removeStoragePath(filename);
  ...
  function showModal(json) {
    ...
    var templateData = {
      ...
      meta: Object.assign({}, json || {}, {
        src: src,
        filename: filename,
        tags: labelManager.indexes.imageLabels[filename],
      }),
      labels: labelManager.labels,
    }
    ...
    // enable remove label buttons
    const removeButtons = target.querySelectorAll('.remove-label-button');
    for (let i = 0; i < removeButtons.length; i++) {
      const removeButton = removeButtons[i];
      removeButton.addEventListener('click', el => {
        const labelName = removeButton.getAttribute('data-label');
        labelManager.removeLabel(labelName, [filename], percent => {
          removeButton.innerHTML = 'Please wait (' + percent.toFixed(2) + '%)';
        }, () => {
          // re-initialize this modal
          removeButton.innerHTML = 'x';
    ...
    // enable add label button
    const addButton = target.querySelector('.apply-label-button');
    addButton.addEventListener('click', () => {
      const labelName = target.querySelector('#label-dropdown').value;
      labelManager.applyLabel(labelName, [filename], percent => {
        addButton.innerHTML = 'Please wait (' + percent.toFixed(2) + '%)';
      }, () => {
        // re-initialize this modal
        addButton.innerHTML = 'Add';
...
LOD.prototype.fetchNextImage = function() {
  ...
  if (Number.isInteger(cellIdx)) {
    ...
    // this image isn't in the cache, so load and cache it
      ...
      image.src = getPath(config.data.dir + '/thumbs/' + data.json.imageNames[cellIdx]);
    };
...
Hotspots.prototype.addEventListeners = function() {
  ...
  this.elems.createHotspot.addEventListener('click', function() {
    ...
    this.json.push({
      ...
      img_name: data.json.imageNames[choose(indices)],
      images: indices,
...
Hotspots.prototype.render = function() {
  ...
  if (this.mesh) world.scene.remove(this.mesh);
  // replace .fits extension with .png
  this.json.forEach(hotspot => {
    hotspot.img_name = hotspot.img_name.replace(/\.fits/ig, '\.png');
  });
  // render the new data
...
Globe.prototype.load = function() {
  ...
  get(getPath('pixplot/json/flat-continents.json'), function(json) {
  ...
  get(getPath('pixplot/json/geographic-features.json'), function(json) {
...
function Tooltip() {
  ...
  this.targets = [
    ...
    {
      elem: document.querySelector('#layout-custom'),
      text: 'Custom layout (Caesar UMAP)',
    },
  ];
...
Welcome.prototype.updateProgress = function() {
  ...
  if (progress == 100 && data.loadedTextures == data.textureCount && world.heightmap && labelManager.ready) {
...
DatasetManager.prototype.createDataset = function (name, successCallback, errorCallback) {
  var request = {
    name: name,
  }
  post('/api/datasets', request, successCallback, errorCallback)
}

DatasetManager.prototype.addImages = function (datasetId, sourceDatasetId, images, progressCallback, successCallback, errorCallback) {
  // prepare batch commands
  const state = { batchesCompleted: 0, batches: 0 }
  const stack = [];
  stack.push(successCallback);
  const batchSize = 250;
  let processed = 0;
  while (true) {
    const batch = images.slice(processed, processed + batchSize);
    if (batch.length === 0)
      break;
    const processBatchFn = () => {
      const request = {
        sourceDatasetId: sourceDatasetId,
        imageNames: batch.map(image => labelManager.indexes.imageNames[image]),
      };
      post('/api/datasets/' + datasetId + '/add-images', request, () => {

        // report progress
        state.batchesCompleted++;
        const percent = state.batchesCompleted / state.batches * 100.0;
        progressCallback(percent);

        // execute next batch command
        const nextFn = stack.pop();
        nextFn();
      }, err => {
        console.log(err);
        errorCallback(err);
...
function LabelManager() {
  this.elems = {
    template: document.querySelector('#labels-template'),
  ...
  this.ready = false;
  this.labels = []; // GET /api/labels

  this.indexes = {
    imageIndexes: {}, // 'D:/.../G002.5+0.0IFx_Mosaic_Mom0_cutout0000001.png': 0
    imageLabels: {}, // 'D:/.../G002.5+0.0IFx_Mosaic_Mom0_cutout0000001.png': ['COMPACT', 'MOSAICING']
    imageNames: {}, // 'D:/.../G002.5+0.0IFx_Mosaic_Mom0_cutout0000001.png': 'G002.5+0.0IFx_Mosaic_Mom0_cutout0000001.png'
    labelImages: {}, // 'COMPACT': ['D:/.../G002.5+0.0IFx_Mosaic_Mom0_cutout0000001.png']
    labelImageIndexes: {}, // 'COMPACT': [1, 2]
    labelIds: {}, // 'COMPACT': 1
    labelColors: {}, // 'COMPACT': '#1678c2'
    colors: {
      black: '#27292a',
      blue: '#1678c2',
      brown: '#a5673f',
...
LabelManager.prototype.addEventListeners = function () {
}

LabelManager.prototype.init = function () {
  console.log('images count: ' + data.json.images.length);

  for (var i = 0; i < data.json.images.length; i++) {
    var image = data.json.images[i];
    this.indexes.imageIndexes[image] = i;
    this.indexes.imageLabels[image] = [];
    this.indexes.imageNames[image] = getNameWoExt(image);
  }

  get('/api/labels', function (labelsResponse) {
    this.labels = JSON.parse(labelsResponse);
    this.labels.forEach(label => {
      this.indexes.labelImages[label.name] = [];
      this.indexes.labelImageIndexes[label.name] = [];
      this.indexes.labelIds[label.name] = label.id;
      this.indexes.labelColors[label.name] = this.indexes.colors[label.color];
    });
    get('/api/datasets/' + window.astroview.datasetId + '/image-labels?displayModeId=' + window.astroview.displayModeId, function (dsLabelsReponse) {
      var imageLabels = JSON.parse(dsLabelsReponse);
      imageLabels.forEach(pair => {
        var imageIndex = this.indexes.imageIndexes[pair.imageName];
        if (!this.indexes.imageLabels[pair.imageName]) {
          this.indexes.imageLabels[pair.imageName] = [];
          console.log('PixPlot is missing image: ' + pair.imageName);
        }
        this.indexes.imageLabels[pair.imageName].push(pair.labelName);
        this.indexes.labelImages[pair.labelName].push(pair.imageName);
        this.indexes.labelImageIndexes[pair.labelName].push(imageIndex);
      });
...
LabelManager.prototype.renderLabelPanel = function () {
  let labelStats = this.labels.map(label => {
    return Object.assign({}, label, {
      color: this.indexes.labelColors[label.name],
      imagesCount: this.indexes.labelImages[label.name].length,
    })
  });
  this.elems.labels.innerHTML = _.template(this.elems.template.innerHTML)({
    labels: labelStats,
  });
  var labelDivs = document.querySelectorAll('.label');
  for (var i = 0; i < labelDivs.length; i++) {
    var labelName = labelDivs[i].querySelector('.label-name').textContent.trim();
    // show the number of cells in this label's cluster
    labelDivs[i].querySelector('.label-bar-inner').style.width = (100 * this.indexes.labelImages[labelName].length / data.cells.length) + '%';
    // add label event listeners each time they are re-rendered
    labelDivs[i].addEventListener('click', function (idx) {
      world.flyToCellImage(this.json[idx].img);
    }.bind(this, i));
    // show the convex hull of a cluster on mouse enter
    labelDivs[i].addEventListener('mouseenter', function (idx) {
      var labelDivsInner = document.querySelectorAll('.label');
      var labelNameInner = labelDivsInner[idx].querySelector('.label-name').textContent.trim();
      this.setLabelHoverBuffer(this.indexes.labelImageIndexes[labelNameInner]);
    }.bind(this, i))
    // remove the convex hull shape on mouseout
    labelDivs[i].addEventListener('mouseleave', function (e) {
      // update the hover cell buffer
      this.setLabelHoverBuffer([]);
      // remove the mesh
      world.scene.remove(this.mesh);
    }.bind(this))
    // prevent newlines when retitling
    labelDivs[i].querySelector('.label-label').addEventListener('keydown', function (i, e) {
      if (e.keyCode == 13) {
        e.stopPropagation();
        e.preventDefault();
        labelDivs[i].querySelector('.label-label').blur()
      }
    }.bind(this, i))
  }
}

LabelManager.prototype.setLabelHoverBuffer = function (arr) {
  // arr contains an array of indices of cells currently selected - create dict for fast lookups
  var d = arr.reduce(function (obj, i) {
    obj[i] = true;
    return obj;
...
LabelManager.prototype.applyLabel = function (label, images, progressCallback, successCallback, errorCallback) {
  // prepare batch commands
  const state = { batchesCompleted: 0, batches: 0 }
  const stack = [];
  stack.push(successCallback);
  ...
  while (true) {
    ...
    const processBatchFn = () => {
      const request = {
        labelId: labelManager.indexes.labelIds[label],
        imageNames: batch.map(image => labelManager.indexes.imageNames[image]),
      };
      post('/api/datasets/' + window.astroview.datasetId + '/image-labels/apply', request, () => {
        images.forEach(image => {
          const imageIndex = labelManager.indexes.imageIndexes[image];
          labelManager.indexes.imageLabels[image].pushIfNotIncluded(label);
          labelManager.indexes.labelImages[label].pushIfNotIncluded(image);
          labelManager.indexes.labelImageIndexes[label].pushIfNotIncluded(imageIndex);
        });
        this.renderLabelPanel();
...
LabelManager.prototype.removeLabel = function (label, images, progressCallback, successCallback, errorCallback) {
  const state = { batchesCompleted: 0, batches: 0 }
  const stack = [];
  stack.push(successCallback);
  const batchSize = 500;
  ...
  while (true) {
    ...
    const processBatchFn = () => {
      ...
      post('/api/datasets/' + window.astroview.datasetId + '/image-labels/remove', request, () => {
        images.forEach(image => {
          var imageIndex = labelManager.indexes.imageIndexes[image];
          labelManager.indexes.imageLabels[image] = labelManager.indexes.imageLabels[image].filter(r => r != label);
          labelManager.indexes.labelImages[label] = labelManager.indexes.labelImages[label].filter(r => r != image);
          labelManager.indexes.labelImageIndexes[label] = labelManager.indexes.labelImageIndexes[label].filter(r => r != imageIndex);
        });
        this.renderLabelPanel();
...
function SearchPanel() {
  this.elems = {
    nameField: document.querySelector('#search-field-name'),
...
SearchPanel.prototype.addEventListeners = function () {
  // search button listener
  this.elems.searchButton.addEventListener('click', function (e) {
    const name = this.elems.nameField.value;
    const image = data.json.images.find(r => r.indexOf(name) > 0);
    world.flyToCellImage(image);
  }.bind(this));
}
...
function getPath(path) {
  ...
  //return base;
  return window.astroview.mapData + path;
}

function removeStoragePath(path) {
  return 'static/storage/' + path.replace(window.astroview.storagePath, '');
}

function getNameWoExt(path) {
  return path.split('/').slice(-1)[0].replace('.png', '');
}
