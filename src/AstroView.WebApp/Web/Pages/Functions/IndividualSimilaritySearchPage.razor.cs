...
public partial class IndividualSimilaritySearchPage
{
    [Parameter]
    ...
    private readonly IndividualSimilaritySearchPagePageVm vm;

    public IndividualSimilaritySearchPage(
        ...
        : base(js, config, asp, nav, dbf)
    {
        ...
        vm = new IndividualSimilaritySearchPagePageVm();
    }
    ...
    {
        if (vm.Parameters.FilePath.Length == 0)
        {
            return;
        }
        ...
        try
        {
            ...
            uploadFileReq.FilePath = vm.Parameters.FilePath;
            var uploadFileRes = await api.UploadFile(uploadFileReq);
            var uid = uploadFileRes.uuid;

            var requestLog = vm.Parameters.FilePath + " | ";
            ...
            request.data_inputs.data = new List<string> { caesarDatasetPath, uid };
            request.data_inputs.format = new List<string> { "abspath", "uid" };
            ...
            var caesarJob = new CaesarJobDbe
            {
                DatasetId = DatasetId,
                UserId = AppVm.UserId,
                AppName = CaesarJob.INDIVIDUAL_SIMILARITY_SEARCH,
            ...
            {
                Type = ChangeType.ExecuteIndividualSimilaritySearch,
                DatasetId = DatasetId,
                UserId = AppVm.UserId,
    ...
    private async Task ShowSelectFileDialog()
    {
        await js.InvokeVoidAsync("showModal");
    }

    private async Task SelectFile(NotedFile file)
    {
        vm.Parameters.FilePath = file.Path;

        await js.InvokeVoidAsync("hideModal");
    }

    private class IndividualSimilaritySearchPagePageVm
    {
        ...
        public IndividualSimilaritySearchPagePageVm()
        {
            DatasetName = "";
            Paging = new Paging();
